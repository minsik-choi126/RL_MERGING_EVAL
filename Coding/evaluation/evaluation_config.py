...
# chat_template: "auto" (모델 tokenizer의 chat_template 사용), "qwen" (Qwen ChatML 하드코딩), "cure" (CURE 포맷)
chat_template = "auto"

# --- 순수 메시지 내용 (chat_template="auto"에서 tokenizer.apply_chat_template으로 감쌈) ---
_auto_system_msg_code = "You are a helpful assistant help user solve problems."
_auto_user_msg_code = """You need to think first then write {{language}} script. {{special_requirements}}\nThis is the problem:\n{{problem}}"""

_auto_system_msg_case = "You are a helpful assistant help user generate test examples for coding tasks."
_auto_user_msg_case = """Given a coding task, instead of providing the final script, your task is to generate a new test example (both input, output and explanation).
This is the problem:\n{{problem}}\n
{{example_intro}}
...
# --- CURE 커스텀 포맷 (ReasonFlux-Coder 학습 시 사용된 포맷) ---
_cure_system_prompts = """<|im_start|>You are a helpful assistant help user solve problems. <|im_end|>\n<|im_start|>User: You need to think first then write {{language}} script. {{special_requirements}}\nThis is the problem:\n{{problem}} <|im_end|>\n<|im_start|>Assistant: """

_cure_system_case_prompts = """<|im_start|>You are a helpful assistant help user generate test examples for coding tasks. <|im_end|>\n<|im_start|>User: Given a coding task, instead of providing the final script, your task is to generate a new test example (both input, output and explanation).
This is the problem:\n{{problem}}\n
{{example_intro}}
You need to provide a new test example. A good test example should be completely accurate and conform to the problem’s format requirements, while also possessing enough discriminative power to distinguish correct code from incorrect code.
Before providing a test example, you must think carefully and reason step by step to derive an input and output you are very confident are correct. For example, start by designing an input you can reliably handle, then compute the output step by step. If you’re unsure about the output, revise or re-design the input to ensure accuracy. Directly providing input/output pairs without this process is discouraged, as it often results in low accuracy.
...
# chat_template에 따라 자동 선택
def _select_prompts(template):
    if template == "cure":
        return _cure_system_prompts, _cure_system_case_prompts
    if template == "auto":
        return None, None  # eval.py에서 tokenizer.apply_chat_template 사용
    return _qwen_system_prompts, _qwen_system_case_prompts

system_prompts, system_case_prompts = _select_prompts(chat_template)
