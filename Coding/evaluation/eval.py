...
def extract_code(full_output):
    # 1st try: ```python ... ``` (preferred)
    matches = re.findall(r"```python(.*?)```", full_output, re.DOTALL)
    if matches:
        return matches[-1].strip()
    # 2nd try: ``` ... ``` (generic code block, no language specifier)
...
def parse_args():
    ...
    parser.add_argument("--chat_template", type=str, default=evaluation_config.chat_template,
                        choices=["auto", "qwen", "cure"],
                        help="Chat template: 'auto' (모델 tokenizer 사용), 'qwen' (ChatML 하드코딩), 'cure' (CURE 포맷)")
    parser.add_argument("--system_prompts", type=str, default=None)
    parser.add_argument("--system_case_prompts", type=str, default=None)
    parser.add_argument("--special_requirements", type=str, default=evaluation_config.special_requirements)
    args = parser.parse_args()
    # --chat_template에 따라 system_prompts 자동 선택 (명시적 --system_prompts 지정 시 그쪽 우선)
    _auto_sp, _auto_scp = evaluation_config._select_prompts(args.chat_template)
    if args.system_prompts is None:
        args.system_prompts = _auto_sp
    if args.system_case_prompts is None:
        args.system_case_prompts = _auto_scp
    return args
...
# obtain prompt
def _apply_auto_template(system_msg, user_msg):
    """chat_template='auto' 모드: tokenizer.apply_chat_template으로 포맷"""
    messages = [
...
def get_scaling_prompt(data_i, method):
    ...
    if method == "sample":
        if chat_template == "auto":
            user_msg = Template(evaluation_config._auto_user_msg_code).render(
                language="python", special_requirements=special_requirements, problem=problem)
            return _apply_auto_template(evaluation_config._auto_system_msg_code, user_msg)
    ...
    if method == "case":
        ...
        if chat_template == "auto":
            user_msg = Template(evaluation_config._auto_user_msg_case).render(
                problem=problem, example_intro=example_intro)
            return _apply_auto_template(evaluation_config._auto_system_msg_case, user_msg)
...
def execute_scripts(outputs_name, num_chunks):
    ...
    if is_final_eval:
        if use_api == False:
            model_short = os.path.basename(pretrained_model.rstrip("/"))
        else:
            model_short = api_model_name.replace("/", "-")
        outputs_result_name = "./results/results-eval-" + model_short + "-" + dataset + ".txt"
    else:
