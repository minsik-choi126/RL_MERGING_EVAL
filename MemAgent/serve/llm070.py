...
class VLLMDeployment:
    def __init__(
        ...
        if engine_args.model.startswith("hdfs:"):
            ...
            import os
            cache_dir = os.path.join(os.environ.get("XDG_CACHE_HOME", os.path.expanduser("~/.cache")), "verl/rlhf")
            local_model_path = copy_local_path_from_hdfs(src=engine_args.model, cache_dir=cache_dir)
...
if __name__ == "__main__":
    ...
    os.chdir(pwd)
    cmd = f"RAY_SERVE_QUEUE_LENGTH_RESPONSE_DEADLINE_S=1 exec serve run --name VLLMMultiDeployment {file}:build_app model={args.model} tensor-parallel-size={args.tp} dtype=float16"
    import subprocess
